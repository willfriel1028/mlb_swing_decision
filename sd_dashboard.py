...
# Scrape statcast data using pybaseball - starts first day of 2026 season, ends at current date
end_dt = date.today().strftime('%Y-%m-%d')
data = statcast(start_dt='2026-03-25', end_dt=end_dt)

# Return clean and filtered dataframe for dashboard to use
def clean_data(data, end_dt):
    ### ATTAIN PLAYER NAMES USING BATTER ID
    # Get unique batter ids as list
    batter_ids = data['batter'].unique().tolist()
    
    # Returns name_first, name_last, key_mlbam
    lookup = playerid_reverse_lookup(batter_ids, key_type='mlbam')
    
    # Merge batter name as a column with our df
    ...
    # Away team if hitting in top of inning, home team if hitting in bottom of inning
    ...
    # Set validated pitch outcomes where a hitter actually makes a swing decision
    ...
    ### GROUPING PITCH TYPES
    # Create 'pitch_group' column to group together pitch types
    fa = ['FF', 'SI', 'FC']
    ...
    # Narrow down columns to only include columns of interest
    ...
    # Drop any columns with NA's that shouldn't have them
    ...
    # Define a strike based on the zone where the pitch crossed home plate (according to statcast)
    strikes = list(range(1,10))
    # Define instances where a hitter decided to swing at a pitch (based on description column)
    ...
    ##### SWING DECISION COLUMNS - CORRECT DECISION IF SWUNG AT STRIKE OR BALL TAKEN, INCORRECT DECISION IF SWUNG AT BALL OR STRIKE TAKEN
    # Define our swing decision columns
    ...
    # Define correct/incorrect swing decision columns
    data4["correct_sd"] = 0
    data4["incorrect_sd"] = 0
    
    # Correct Decisions (Swung at strike or didn't swing at ball)
    data4.loc[(data4["strike_swing"] == 1) | (data4["ball_taken"] == 1), "correct_sd"] = 1
    # Incorrect Decisions (Took a strike or swung at ball
    data4.loc[(data4["strike_taken"] == 1) | (data4["ball_swing"] == 1), "incorrect_sd"] = 1


    # Assign 'S' as batter stance if switch hitter
...
# Calls clean_data function, saves new dataset as 'df'
